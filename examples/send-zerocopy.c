...
static void setup_sockaddr(int domain, const char* str_addr, struct sockaddr_storage* sockaddr) {
    ...
    switch (domain) {
        ...
        case PF_INET6:
            ...
            break;
        default: t_error(1, 0, "illegal domain");
    }
}
...
static inline struct io_uring_cqe* wait_cqe_fast(struct io_uring* ring) {
    ...
    io_uring_for_each_cqe(ring, head, cqe) return cqe;
    ...
    do {
        ...
        for (i = 0; i < cfg_nr_reqs; i++) {
            ...
            if (!cfg_zc)
                ...
                io_uring_prep_send_zc(sqe, fd, payload, cfg_payload_len, msg_flags, 0);
                if (cfg_fixed_buf) {
        ...
        for (i = 0; i < cfg_nr_reqs; i++) {
            ...
            if (cqe->res >= 0) {
                td->packets++;
                td->bytes += cqe->res;
            } else if (cqe->res == -ECONNREFUSED || cqe->res == -EPIPE || cqe->res == -ECONNRESET) {
...
static void usage(const char* filepath) {
    t_error(1,
            0,
            "Usage: %s [-n<N>] [-z<val>] [-s<payload size>] "
            "(-4|-6) [-t<time s>] -D<dst_ip> udp",
            filepath);
}

static void parse_opts(int argc, char** argv) {
    const int max_payload_len =
      IP_MAXPACKET - sizeof(struct ipv6hdr) - sizeof(struct tcphdr) - 40 /* max tcp options */;
    int   c;
    char* daddr = NULL;
    ...
    while ((c = getopt(argc, argv, "46D:p:s:t:n:z:b:l:dC:T:R")) != -1) {
        switch (c) {
            ...
            case '6':
                ...
                break;
            case 'D': daddr = optarg; break;
            case 'p': cfg_port = strtoul(optarg, NULL, 0); break;
            case 's': cfg_payload_len = strtoul(optarg, NULL, 0); break;
            case 't': cfg_runtime_ms = 200 + strtoul(optarg, NULL, 10) * 1000; break;
            case 'n': cfg_nr_reqs = strtoul(optarg, NULL, 0); break;
            case 'z': cfg_zc = strtoul(optarg, NULL, 0); break;
            case 'b': cfg_fixed_buf = strtoul(optarg, NULL, 0); break;
            case 'l': cfg_hugetlb = strtoul(optarg, NULL, 0); break;
            case 'd': cfg_defer_taskrun = 1; break;
            case 'C': cfg_cpu = strtol(optarg, NULL, 0); break;
            case 'T':
                cfg_nr_threads = strtol(optarg, NULL, 0);
                if (cfg_nr_threads > MAX_THREADS)
                    t_error(1, 0, "too many threads\n");
                break;
            case 'R': cfg_rx = 1; break;
        }
    }
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < cfg_nr_threads; i++)
        pthread_create(&threads[i].thread, NULL, !cfg_rx ? do_test : do_rx, &threads[i]);
    ...
    fprintf(stderr,
            "packets=%llu (MB=%llu), rps=%llu (MB/s=%llu)\n",
            packets,
            bytes >> 20,
