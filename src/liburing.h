...
IOURINGINLINE int
do_register(struct io_uring* ring, unsigned int opcode, const void* arg, unsigned int nr_args) noexcept {
    int fd;
...
IOURINGINLINE int io_uring_register_buffers_tags(struct io_uring*    ring,
                                                 const struct iovec* iovecs,
                                                 const __u64*        tags,
                                                 unsigned            nr) noexcept {
...
IOURINGINLINE int
io_uring_register_buffers(struct io_uring* ring, const struct iovec* iovecs, unsigned nr_iovecs) noexcept {
    return do_register(ring, IORING_REGISTER_BUFFERS, iovecs, nr_iovecs);
}

IOURINGINLINE int io_uring_unregister_buffers(struct io_uring* ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_BUFFERS, nullptr, 0);
}

IOURINGINLINE int io_uring_register_files_update_tag(struct io_uring* ring,
                                                     unsigned         off,
                                                     const int*       files,
                                                     const __u64*     tags,
 ...
 */
IOURINGINLINE int io_uring_register_files_update(struct io_uring* ring,
                                                 unsigned         off,
                                                 const int*       files,
                                                 unsigned         nr_files) noexcept {
...
IOURINGINLINE int io_uring_register_files_sparse(struct io_uring* ring, unsigned nr) noexcept {
    struct io_uring_rsrc_register reg = {
      .flags = IORING_RSRC_REGISTER_SPARSE,
      .nr    = nr,
    };
...
IOURINGINLINE int io_uring_register_files_tags(struct io_uring* ring,
                                               const int*       files,
                                               const __u64*     tags,
                                               unsigned         nr) noexcept {
    struct io_uring_rsrc_register reg = {
...
IOURINGINLINE int
io_uring_register_files(struct io_uring* ring, const int* files, unsigned nr_files) noexcept {
    int ret, did_increase = 0;
...
IOURINGINLINE int io_uring_unregister_files(struct io_uring* ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_FILES, nullptr, 0);
}

IOURINGINLINE int io_uring_register_eventfd(struct io_uring* ring, int event_fd) noexcept {
    return do_register(ring, IORING_REGISTER_EVENTFD, &event_fd, 1);
}

IOURINGINLINE int io_uring_unregister_eventfd(struct io_uring* ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_EVENTFD, nullptr, 0);
}

IOURINGINLINE int io_uring_register_eventfd_async(struct io_uring* ring, int event_fd) noexcept {
    return do_register(ring, IORING_REGISTER_EVENTFD_ASYNC, &event_fd, 1);
}

IOURINGINLINE int
io_uring_register_probe(struct io_uring* ring, struct io_uring_probe* p, unsigned int nr_ops) noexcept {
    return do_register(ring, IORING_REGISTER_PROBE, p, nr_ops);
}

IOURINGINLINE int io_uring_register_personality(struct io_uring* ring) noexcept {
    return do_register(ring, IORING_REGISTER_PERSONALITY, nullptr, 0);
}

IOURINGINLINE int io_uring_unregister_personality(struct io_uring* ring, int id) noexcept {
    return do_register(ring, IORING_UNREGISTER_PERSONALITY, nullptr, id);
}

IOURINGINLINE int io_uring_register_restrictions(struct io_uring*             ring,
                                                 struct io_uring_restriction* res,
                                                 unsigned int                 nr_res) noexcept {
    return do_register(ring, IORING_REGISTER_RESTRICTIONS, res, nr_res);
}

IOURINGINLINE int io_uring_enable_rings(struct io_uring* ring) noexcept {
...
IOURINGINLINE int io_uring_register_iowq_max_workers(struct io_uring* ring, unsigned int* val) noexcept {
...
IOURINGINLINE int io_uring_register_ring_fd(struct io_uring* ring) noexcept {
    ...
    if (ret == 1) {
        ring->enter_ring_fd = uring_static_cast(int, up.offset);
...
IOURINGINLINE int io_uring_close_ring_fd(struct io_uring* ring) noexcept {
    if (!(ring->features & IORING_FEAT_REG_REG_RING))
        return -EOPNOTSUPP;
    if (!(ring->int_flags & INT_FLAG_REG_RING))
        return -EINVAL;
...
/**
 * return an allocated io_uring_probe structure, or nullptr if probe fails (for
...
IOURINGINLINE int io_uring_opcode_supported(const struct io_uring_probe* p, int op) noexcept {
    if (op > p->last_op)
        return 0;
...
IOURINGINLINE int
get_sq_cq_entries(unsigned entries, struct io_uring_params* p, unsigned* sq, unsigned* cq) noexcept {
    unsigned cq_entries;
 ...
 */
IOURINGINLINE int io_uring_alloc_huge(unsigned                entries,
...
constexpr IOURINGINLINE void io_uring_setup_ring_pointers(struct io_uring_params* p,
                                                          struct io_uring_sq*     sq,
...
IOURINGINLINE int internal__io_uring_queue_init_params(unsigned                entries,
 ...
 * host the ring.
 */
IOURINGINLINE int io_uring_queue_init_mem(unsigned                entries,
                                          struct io_uring*        ring,
                                          struct io_uring_params* p,
                                          void*                   buf,
                                          size_t                  buf_size) noexcept {
    ...
    return internal__io_uring_queue_init_params(entries, ring, p, buf, buf_size);
}

IOURINGINLINE int
io_uring_queue_init_params(unsigned entries, struct io_uring* ring, struct io_uring_params* p) noexcept {
    int const ret = internal__io_uring_queue_init_params(entries, ring, p, nullptr, 0);
    return ret >= 0 ? 0 : ret;
}
...
uring__cold IOURINGINLINE int io_uring_ring_dontfork(struct io_uring* ring) noexcept {
    ...
    size_t len = sizeof(struct io_uring_sqe);
 ...
 */
IOURINGINLINE unsigned io_uring_cq_ready(const struct io_uring* ring) noexcept {
...
IOURINGINLINE int io_uring_get_events(struct io_uring* ring) noexcept {
 ...
 */
IOURINGINLINE unsigned
io_uring_peek_batch_cqe(struct io_uring* ring, struct io_uring_cqe** cqes, unsigned count) noexcept {
    unsigned ready;
...
#define LIBURING_UDATA_TIMEOUT              (uring_static_cast(__u64, -1))
 ...
 */
IOURINGINLINE int internal__io_uring_peek_cqe(struct io_uring*      ring,
                                              struct io_uring_cqe** cqe_ptr,
                                              unsigned*             nr_available) noexcept {
...
IOURINGINLINE int internal_io_uring_get_cqe(struct io_uring*      ring,
                                            struct io_uring_cqe** cqe_ptr,
                                            struct get_data*      data) noexcept {
    ...
    do {
        ...
        ret = internal__io_uring_peek_cqe(ring, &cqe, &nr_available);
        if (ret) {
            if (!err)
                err = ret;
            break;
 ...
 */
IOURINGINLINE int io_uring_wait_cqes_new(struct io_uring*          ring,
                                         struct io_uring_cqe**     cqe_ptr,
                                         unsigned                  wait_nr,
 ...
 *
 * Returns a vacant sqe, or nullptr if we're full.
 */
IOURINGINLINE struct io_uring_sqe* internal_io_uring_get_sqe(struct io_uring* ring) noexcept {
    struct io_uring_sq* sq = &ring->sq;
...
struct io_uring_sqe* io_uring_get_sqe(struct io_uring* ring) {
    return internal_io_uring_get_sqe(ring);
}
 ...
 */
IOURINGINLINE unsigned internal__io_uring_flush_sq(struct io_uring* ring) noexcept {
    struct io_uring_sq* sq   = &ring->sq;
 ...
 */
IOURINGINLINE int internal__io_uring_get_cqe(struct io_uring*      ring,
                                             struct io_uring_cqe** cqe_ptr,
                                             unsigned              submit,
                                             unsigned              wait_nr,
                                             sigset_t*             sigmask) noexcept {
...
IOURINGINLINE int internal__io_uring_submit_timeout(struct io_uring*          ring,
                                                    unsigned                  wait_nr,
                                                    struct __kernel_timespec* ts) noexcept;

IOURINGINLINE int io_uring_wait_cqes(struct io_uring*          ring,
                                     struct io_uring_cqe**     cqe_ptr,
                                     unsigned                  wait_nr,
                                     struct __kernel_timespec* ts,
                                     sigset_t*                 sigmask) noexcept {
 ...
 * '1' as the wait_nr.
 */
IOURINGINLINE int io_uring_wait_cqe_timeout(struct io_uring*          ring,
                                            struct io_uring_cqe**     cqe_ptr,
                                            struct __kernel_timespec* ts) noexcept {
    return io_uring_wait_cqes(ring, cqe_ptr, 1, ts, nullptr);
}
 ...
 */
IOURINGINLINE int internal__io_uring_submit(struct io_uring* ring,
                                            unsigned         submitted,
                                            unsigned         wait_nr,
...
IOURINGINLINE int internal__io_uring_submit_and_wait(struct io_uring* ring, unsigned wait_nr) noexcept {
 ...
 * Returns number of sqes submitted
 */
IOURINGINLINE int io_uring_submit(struct io_uring* ring) noexcept {
 ...
 * Returns number of sqes submitted
 */
IOURINGINLINE int io_uring_submit_and_wait(struct io_uring* ring, unsigned wait_nr) noexcept {
    return internal__io_uring_submit_and_wait(ring, wait_nr);
}

IOURINGINLINE int io_uring_submit_and_get_events(struct io_uring* ring) noexcept {
    return internal__io_uring_submit(ring, internal__io_uring_flush_sq(ring), 0, true);
}
...
constexpr IOURINGINLINE void io_uring_prep_timeout(struct io_uring_sqe*      sqe,
                                                   struct __kernel_timespec* ts,
                                                   unsigned                  count,
...
constexpr IOURINGINLINE void io_uring_buf_ring_init(struct io_uring_buf_ring* br) noexcept {
    br->tail = 0;
}
...
uring__cold IOURINGINLINE ssize_t io_uring_mlock_size_params(unsigned                entries,
    ...
    return uring_static_cast(ssize_t, rings_size(p, sq, cq_entries, page_size));
}
...
#if defined(__hppa__)
IOURINGINLINE struct io_uring_buf_ring*
br_setup(struct io_uring* ring, unsigned int nentries, int bgid, unsigned int flags, int* ret) noexcept {
#    ifdef __cplusplus
    struct io_uring_buf_reg reg {};
#    else
    struct io_uring_buf_reg reg;
 ...
 */
IOURINGINLINE struct io_uring_buf_ring* io_uring_setup_buf_ring(struct io_uring* ring,
...
IOURINGINLINE int io_uring_free_buf_ring(struct io_uring*          ring,
                                         struct io_uring_buf_ring* br,
                                         unsigned int              nentries,
                                         int                       bgid) noexcept {
 ...
 */
IOURINGINLINE void io_uring_cqe_seen(struct io_uring* ring, struct io_uring_cqe* cqe) noexcept {
    if (cqe)
        io_uring_cq_advance(ring, 1);
}
 ...
 */
IOURINGINLINE void io_uring_sqe_set_data(struct io_uring_sqe* sqe, void* data) noexcept {
    sqe->user_data = uring_reinterpret_cast(unsigned long, data);
}

IOURINGINLINE void* io_uring_cqe_get_data(const struct io_uring_cqe* cqe) noexcept {
    return uring_reinterpret_cast(void*, uring_reinterpret_cast(uintptr_t, cqe->user_data));
}
 ...
 */
constexpr IOURINGINLINE void io_uring_sqe_set_data64(struct io_uring_sqe* sqe, __u64 data) noexcept {
    sqe->user_data = data;
}

constexpr IOURINGINLINE __u64 io_uring_cqe_get_data64(const struct io_uring_cqe* cqe) noexcept {
...
constexpr IOURINGINLINE void io_uring_sqe_set_flags(struct io_uring_sqe* sqe, unsigned flags) noexcept {
    sqe->flags = uring_static_cast(__u8, flags);
}

constexpr IOURINGINLINE void internal__io_uring_set_target_fixed_file(struct io_uring_sqe* sqe,
                                                                      unsigned int file_index) noexcept {
    /* 0 means no fixed files, indexes should be encoded as "index + 1" */
    sqe->file_index = file_index + 1;
}

constexpr IOURINGINLINE void io_uring_prep_rw(int                  op,
                                              struct io_uring_sqe* sqe,
                                              int                  fd,
                                              const void*          addr,
                                              unsigned             len,
 ...
 */
constexpr IOURINGINLINE void io_uring_prep_splice(struct io_uring_sqe* sqe,
                                                  int                  fd_in,
                                                  int64_t              off_in,
                                                  int                  fd_out,
                                                  int64_t              off_out,
    ...
    io_uring_prep_rw(IORING_OP_SPLICE, sqe, fd_out, nullptr, nbytes, uring_static_cast(__u64, off_out));
    sqe->splice_off_in = uring_static_cast(__u64, off_in);
    sqe->splice_fd_in  = fd_in;
    sqe->splice_flags  = splice_flags;
}

constexpr IOURINGINLINE void io_uring_prep_tee(struct io_uring_sqe* sqe,
                                               int                  fd_in,
                                               int                  fd_out,
                                               unsigned int         nbytes,
                                               unsigned int         splice_flags) noexcept {
...
constexpr IOURINGINLINE void io_uring_prep_readv(struct io_uring_sqe* sqe,
                                                 int                  fd,
                                                 const struct iovec*  iovecs,
                                                 unsigned             nr_vecs,
                                                 __u64                offset) noexcept {
...
constexpr IOURINGINLINE void io_uring_prep_readv2(struct io_uring_sqe* sqe,
                                                  int                  fd,
                                                  const struct iovec*  iovecs,
                                                  unsigned             nr_vecs,
                                                  __u64                offset,
...
constexpr IOURINGINLINE void io_uring_prep_read_fixed(struct io_uring_sqe* sqe,
                                                      int                  fd,
                                                      void*                buf,
                                                      unsigned             nbytes,
                                                      __u64                offset,
...
constexpr IOURINGINLINE void io_uring_prep_writev(struct io_uring_sqe* sqe,
                                                  int                  fd,
                                                  const struct iovec*  iovecs,
                                                  unsigned             nr_vecs,
                                                  __u64                offset) noexcept {
...
constexpr IOURINGINLINE void io_uring_prep_writev2(struct io_uring_sqe* sqe,
                                                   int                  fd,
                                                   const struct iovec*  iovecs,
                                                   unsigned             nr_vecs,
                                                   __u64                offset,
...
constexpr IOURINGINLINE void io_uring_prep_write_fixed(struct io_uring_sqe* sqe,
                                                       int                  fd,
                                                       const void*          buf,
                                                       unsigned             nbytes,
                                                       __u64                offset,
...
constexpr IOURINGINLINE void
io_uring_prep_recvmsg(struct io_uring_sqe* sqe, int fd, struct msghdr* msg, unsigned flags) noexcept {
    io_uring_prep_rw(IORING_OP_RECVMSG, sqe, fd, msg, 1, 0);
    sqe->msg_flags = flags;
}

constexpr IOURINGINLINE void io_uring_prep_recvmsg_multishot(struct io_uring_sqe* sqe,
                                                             int                  fd,
                                                             struct msghdr*       msg,
                                                             unsigned             flags) noexcept {
...
constexpr IOURINGINLINE void
io_uring_prep_sendmsg(struct io_uring_sqe* sqe, int fd, const struct msghdr* msg, unsigned flags) noexcept {
    io_uring_prep_rw(IORING_OP_SENDMSG, sqe, fd, msg, 1, 0);
    sqe->msg_flags = flags;
}

constexpr IOURINGINLINE unsigned internal__io_uring_prep_poll_mask(unsigned poll_mask) noexcept {
...
constexpr IOURINGINLINE void
io_uring_prep_poll_add(struct io_uring_sqe* sqe, int fd, unsigned poll_mask) noexcept {
    io_uring_prep_rw(IORING_OP_POLL_ADD, sqe, fd, nullptr, 0, 0);
    sqe->poll32_events = internal__io_uring_prep_poll_mask(poll_mask);
}

constexpr IOURINGINLINE void
io_uring_prep_poll_multishot(struct io_uring_sqe* sqe, int fd, unsigned poll_mask) noexcept {
    io_uring_prep_poll_add(sqe, fd, poll_mask);
    sqe->len = IORING_POLL_ADD_MULTI;
}

constexpr IOURINGINLINE void io_uring_prep_poll_remove(struct io_uring_sqe* sqe, __u64 user_data) noexcept {
    io_uring_prep_rw(IORING_OP_POLL_REMOVE, sqe, -1, nullptr, 0, 0);
    sqe->addr = user_data;
}

constexpr IOURINGINLINE void io_uring_prep_poll_update(struct io_uring_sqe* sqe,
                                                       __u64                old_user_data,
                                                       __u64                new_user_data,
                                                       unsigned             poll_mask,
                                                       unsigned             flags) noexcept {
...
constexpr IOURINGINLINE void
io_uring_prep_fsync(struct io_uring_sqe* sqe, int fd, unsigned fsync_flags) noexcept {
    io_uring_prep_rw(IORING_OP_FSYNC, sqe, fd, nullptr, 0, 0);
    sqe->fsync_flags = fsync_flags;
}

constexpr IOURINGINLINE void io_uring_prep_nop(struct io_uring_sqe* sqe) noexcept {
...
constexpr IOURINGINLINE void
io_uring_prep_timeout_remove(struct io_uring_sqe* sqe, __u64 user_data, unsigned flags) noexcept {
...
IOURINGINLINE void io_uring_prep_timeout_update(struct io_uring_sqe*      sqe,
                                                ...
                                                __u64                     user_data,
                                                unsigned                  flags) noexcept {
    io_uring_prep_rw(IORING_OP_TIMEOUT_REMOVE, sqe, -1, nullptr, 0, uring_reinterpret_cast(uintptr_t, ts));
    sqe->addr          = user_data;
    sqe->timeout_flags = flags | IORING_TIMEOUT_UPDATE;
}

IOURINGINLINE void io_uring_prep_accept(struct io_uring_sqe* sqe,
                                        int                  fd,
                                        struct sockaddr*     addr,
                                        socklen_t*           addrlen,
                                        int                  flags) noexcept {
    io_uring_prep_rw(IORING_OP_ACCEPT,
                     sqe,
                     fd,
                     addr,
                     0,
                     uring_static_cast(__u64, uring_reinterpret_cast(unsigned long, addrlen)));
    sqe->accept_flags = uring_static_cast(__u32, flags);
}

/* accept directly into the fixed file table */
IOURINGINLINE void io_uring_prep_accept_direct(struct io_uring_sqe* sqe,
                                               int                  fd,
                                               struct sockaddr*     addr,
                                               socklen_t*           addrlen,
                                               int                  flags,
...
IOURINGINLINE void io_uring_prep_multishot_accept_direct(struct io_uring_sqe* sqe,
    ...
    internal__io_uring_set_target_fixed_file(sqe, IORING_FILE_INDEX_ALLOC - 1);
}

constexpr IOURINGINLINE void
io_uring_prep_cancel64(struct io_uring_sqe* sqe, __u64 user_data, int flags) noexcept {
    io_uring_prep_rw(IORING_OP_ASYNC_CANCEL, sqe, -1, nullptr, 0, 0);
    sqe->addr         = user_data;
    sqe->cancel_flags = (__u32) flags;
}

IOURINGINLINE void io_uring_prep_cancel(struct io_uring_sqe* sqe, void* user_data, int flags) noexcept {
    io_uring_prep_cancel64(sqe,
                           uring_static_cast(__u64, uring_reinterpret_cast(uintptr_t, user_data)),
                           flags);
}

constexpr IOURINGINLINE void
io_uring_prep_cancel_fd(struct io_uring_sqe* sqe, int fd, unsigned int flags) noexcept {
    io_uring_prep_rw(IORING_OP_ASYNC_CANCEL, sqe, fd, nullptr, 0, 0);
    sqe->cancel_flags = uring_static_cast(__u32, flags) | IORING_ASYNC_CANCEL_FD;
}

constexpr IOURINGINLINE void
io_uring_prep_link_timeout(struct io_uring_sqe* sqe, struct __kernel_timespec* ts, unsigned flags) noexcept {
    io_uring_prep_rw(IORING_OP_LINK_TIMEOUT, sqe, -1, ts, 1, 0);
    sqe->timeout_flags = flags;
}

constexpr IOURINGINLINE void io_uring_prep_connect(struct io_uring_sqe*   sqe,
                                                   int                    fd,
...
constexpr IOURINGINLINE void
io_uring_prep_fallocate(struct io_uring_sqe* sqe, int fd, int mode, __u64 offset, __u64 len) noexcept {
    io_uring_prep_rw(IORING_OP_FALLOCATE, sqe, fd, 0, uring_static_cast(unsigned int, mode), offset);
    sqe->addr = len;
}

constexpr IOURINGINLINE void
io_uring_prep_openat(struct io_uring_sqe* sqe, int dfd, const char* path, int flags, mode_t mode) noexcept {
    io_uring_prep_rw(IORING_OP_OPENAT, sqe, dfd, path, mode, 0);
    sqe->open_flags = uring_static_cast(__u32, flags);
}

/* open directly into the fixed file table */
constexpr IOURINGINLINE void io_uring_prep_openat_direct(struct io_uring_sqe* sqe,
                                                         int                  dfd,
                                                         const char*          path,
                                                         int                  flags,
                                                         mode_t               mode,
...
constexpr IOURINGINLINE void io_uring_prep_close(struct io_uring_sqe* sqe, int fd) noexcept {
    io_uring_prep_rw(IORING_OP_CLOSE, sqe, fd, nullptr, 0, 0);
}

constexpr IOURINGINLINE void io_uring_prep_close_direct(struct io_uring_sqe* sqe,
                                                        unsigned             file_index) noexcept {
...
constexpr IOURINGINLINE void
io_uring_prep_read(struct io_uring_sqe* sqe, int fd, void* buf, unsigned nbytes, __u64 offset) noexcept {
    io_uring_prep_rw(IORING_OP_READ, sqe, fd, buf, nbytes, offset);
}

constexpr IOURINGINLINE void io_uring_prep_write(struct io_uring_sqe* sqe,
                                                 int                  fd,
                                                 const void*          buf,
                                                 unsigned             nbytes,
                                                 __u64                offset) noexcept {
...
IOURINGINLINE void io_uring_prep_statx(struct io_uring_sqe* sqe,
                                       int                  dfd,
                                       const char*          path,
                                       int                  flags,
                                       unsigned             mask,
    ...
    io_uring_prep_rw(IORING_OP_STATX,
                     ...
                     path,
                     mask,
                     uring_static_cast(__u64, uring_reinterpret_cast(unsigned long, statxbuf)));
    sqe->statx_flags = uring_static_cast(__u32, flags);
}
...
IOURINGINLINE void io_uring_prep_send_set_addr(struct io_uring_sqe*   sqe,
                                               ...
                                               __u16                  addr_len) noexcept {
    sqe->addr2    = uring_reinterpret_cast(unsigned long, uring_reinterpret_cast(const void*, dest_addr));
    sqe->addr_len = addr_len;
}

IOURINGINLINE void io_uring_prep_sendto(struct io_uring_sqe*   sqe,
                                        int                    sockfd,
                                        const void*            buf,
                                        size_t                 len,
                                        int                    flags,
...
constexpr IOURINGINLINE void io_uring_prep_send_zc(struct io_uring_sqe* sqe,
                                                   int                  sockfd,
                                                   const void*          buf,
                                                   size_t               len,
                                                   int                  flags,
...
constexpr IOURINGINLINE void io_uring_prep_send_zc_fixed(struct io_uring_sqe* sqe,
                                                         int                  sockfd,
                                                         const void*          buf,
                                                         size_t               len,
                                                         int                  flags,
...
constexpr IOURINGINLINE void io_uring_prep_sendmsg_zc(struct io_uring_sqe* sqe,
                                                      int                  fd,
...
constexpr IOURINGINLINE void
io_uring_prep_recv(struct io_uring_sqe* sqe, int sockfd, void* buf, size_t len, int flags) noexcept {
    io_uring_prep_rw(IORING_OP_RECV, sqe, sockfd, buf, uring_static_cast(__u32, len), 0);
    sqe->msg_flags = uring_static_cast(__u32, flags);
}

constexpr IOURINGINLINE void io_uring_prep_recv_multishot(struct io_uring_sqe* sqe,
                                                          int                  sockfd,
                                                          void*                buf,
                                                          size_t               len,
                                                          int                  flags) noexcept {
...
io_uring_recvmsg_validate(void* buf, int buf_len, struct msghdr* msgh) noexcept {
    unsigned long const header = uring_static_cast(unsigned long, msgh->msg_controllen) +
                                 uring_static_cast(unsigned long, msgh->msg_namelen) +
                                 sizeof(struct io_uring_recvmsg_out);
    if (buf_len < 0 || uring_static_cast(unsigned long, buf_len) < header)
        return nullptr;
    return uring_reinterpret_cast(struct io_uring_recvmsg_out*, buf);
}

IOURINGINLINE void* io_uring_recvmsg_name(struct io_uring_recvmsg_out* o) noexcept {
    return uring_reinterpret_cast(void*, &o[1]);
}

IOURINGINLINE struct cmsghdr* io_uring_recvmsg_cmsg_firsthdr(struct io_uring_recvmsg_out* o,
                                                             struct msghdr*               msgh) noexcept {
    if (o->controllen < sizeof(struct cmsghdr))
        return nullptr;

    return uring_reinterpret_cast(
      struct cmsghdr*,
      (uring_reinterpret_cast(unsigned char*, io_uring_recvmsg_name(o)) + msgh->msg_namelen));
}

IOURINGINLINE struct cmsghdr* io_uring_recvmsg_cmsg_nexthdr(struct io_uring_recvmsg_out* o,
                                                            struct msghdr*               msgh,
                                                            struct cmsghdr*              cmsg) noexcept {

    if (cmsg->cmsg_len < sizeof(struct cmsghdr))
        return nullptr;
    unsigned char* end =
      uring_reinterpret_cast(unsigned char*, io_uring_recvmsg_cmsg_firsthdr(o, msgh)) + o->controllen;
    cmsg = (struct cmsghdr*) ((unsigned char*) cmsg + CMSG_ALIGN(cmsg->cmsg_len));

    if (uring_reinterpret_cast(unsigned char*, cmsg + 1) > end)
        return nullptr;
    if (uring_reinterpret_cast(unsigned char*, cmsg) + CMSG_ALIGN(cmsg->cmsg_len) > end)
        return nullptr;

    return cmsg;
}

IOURINGINLINE void* io_uring_recvmsg_payload(struct io_uring_recvmsg_out* o, struct msghdr* msgh) noexcept {
    return uring_reinterpret_cast(void*,
                                  (uring_reinterpret_cast(unsigned char*, io_uring_recvmsg_name(o)) +
                                   msgh->msg_namelen + msgh->msg_controllen));
}

IOURINGINLINE unsigned int
io_uring_recvmsg_payload_length(struct io_uring_recvmsg_out* o, int buf_len, struct msghdr* msgh) noexcept {
...
IOURINGINLINE void
io_uring_prep_openat2(struct io_uring_sqe* sqe, int dfd, const char* path, struct open_how* how) noexcept {
    io_uring_prep_rw(IORING_OP_OPENAT2,
                     sqe,
                     dfd,
                     path,
                     sizeof(*how),
                     uring_static_cast(uint64_t, uring_reinterpret_cast(uintptr_t, how)));
}
...
constexpr IOURINGINLINE void
io_uring_prep_epoll_ctl(struct io_uring_sqe* sqe, int epfd, int fd, int op, struct epoll_event* ev) noexcept {
    io_uring_prep_rw(IORING_OP_EPOLL_CTL,
                     sqe,
                     epfd,
                     ev,
                     uring_static_cast(__u32, op),
                     uring_static_cast(__u32, fd));
}

constexpr IOURINGINLINE void io_uring_prep_provide_buffers(struct io_uring_sqe* sqe,
                                                           void*                addr,
                                                           int                  len,
                                                           int                  nr,
                                                           int                  bgid,
                                                           int                  bid) noexcept {
    io_uring_prep_rw(IORING_OP_PROVIDE_BUFFERS,
                     sqe,
                     nr,
                     addr,
                     uring_static_cast(__u32, len),
                     uring_static_cast(__u64, bid));
    sqe->buf_group = uring_static_cast(__u16, bgid);
}

constexpr IOURINGINLINE void
io_uring_prep_remove_buffers(struct io_uring_sqe* sqe, int nr, int bgid) noexcept {
    io_uring_prep_rw(IORING_OP_REMOVE_BUFFERS, sqe, nr, nullptr, 0, 0);
    sqe->buf_group = uring_static_cast(__u16, bgid);
}

constexpr IOURINGINLINE void io_uring_prep_shutdown(struct io_uring_sqe* sqe, int fd, int how) noexcept {
    io_uring_prep_rw(IORING_OP_SHUTDOWN, sqe, fd, nullptr, uring_static_cast(__u32, how), 0);
}

constexpr IOURINGINLINE void
io_uring_prep_unlinkat(struct io_uring_sqe* sqe, int dfd, const char* path, int flags) noexcept {
    io_uring_prep_rw(IORING_OP_UNLINKAT, sqe, dfd, path, 0, 0);
    sqe->unlink_flags = uring_static_cast(__u32, flags);
}

constexpr IOURINGINLINE void
io_uring_prep_unlink(struct io_uring_sqe* sqe, const char* path, int flags) noexcept {
    io_uring_prep_unlinkat(sqe, AT_FDCWD, path, flags);
}

IOURINGINLINE void io_uring_prep_renameat(struct io_uring_sqe* sqe,
                                          int                  olddfd,
                                          const char*          oldpath,
                                          int                  newdfd,
                                          const char*          newpath,
    ...
    io_uring_prep_rw(IORING_OP_RENAMEAT,
                     ...
                     olddfd,
                     oldpath,
                     uring_reinterpret_cast(__u32, newdfd),
                     uring_static_cast(uint64_t, uring_reinterpret_cast(uintptr_t, newpath)));
    sqe->rename_flags = uring_static_cast(__u32, flags);
}

constexpr IOURINGINLINE void
io_uring_prep_rename(struct io_uring_sqe* sqe, const char* oldpath, const char* newpath) noexcept {
    io_uring_prep_renameat(sqe, AT_FDCWD, oldpath, AT_FDCWD, newpath, 0);
}

constexpr IOURINGINLINE void io_uring_prep_sync_file_range(struct io_uring_sqe* sqe,
                                                           int                  fd,
                                                           unsigned             len,
                                                           __u64                offset,
                                                           int                  flags) noexcept {
    io_uring_prep_rw(IORING_OP_SYNC_FILE_RANGE, sqe, fd, nullptr, len, offset);
    sqe->sync_range_flags = uring_static_cast(__u32, flags);
}

constexpr IOURINGINLINE void
io_uring_prep_mkdirat(struct io_uring_sqe* sqe, int dfd, const char* path, mode_t mode) noexcept {
    io_uring_prep_rw(IORING_OP_MKDIRAT, sqe, dfd, path, mode, 0);
}

constexpr IOURINGINLINE void
io_uring_prep_mkdir(struct io_uring_sqe* sqe, const char* path, mode_t mode) noexcept {
...
IOURINGINLINE void io_uring_prep_symlinkat(struct io_uring_sqe* sqe,
    ...
    io_uring_prep_rw(IORING_OP_SYMLINKAT,
                     ...
                     target,
                     0,
                     uring_static_cast(uint64_t, uring_reinterpret_cast(uintptr_t, linkpath)));
}

IOURINGINLINE void
io_uring_prep_symlink(struct io_uring_sqe* sqe, const char* target, const char* linkpath) noexcept {
    io_uring_prep_symlinkat(sqe, target, AT_FDCWD, linkpath);
}

IOURINGINLINE void io_uring_prep_linkat(struct io_uring_sqe* sqe,
                                        int                  olddfd,
                                        const char*          oldpath,
                                        int                  newdfd,
                                        const char*          newpath,
    ...
    io_uring_prep_rw(IORING_OP_LINKAT,
                     ...
                     uring_reinterpret_cast(__u32, newdfd),
                     uring_static_cast(uint64_t, uring_reinterpret_cast(uintptr_t, newpath)));
    sqe->hardlink_flags = uring_static_cast(__u32, flags);
}

IOURINGINLINE void
io_uring_prep_link(struct io_uring_sqe* sqe, const char* oldpath, const char* newpath, int flags) noexcept {
    io_uring_prep_linkat(sqe, AT_FDCWD, oldpath, AT_FDCWD, newpath, flags);
}

constexpr IOURINGINLINE void io_uring_prep_msg_ring_cqe_flags(struct io_uring_sqe* sqe,
                                                              int                  fd,
                                                              unsigned int         len,
                                                              __u64                data,
                                                              unsigned int         flags,
...
constexpr IOURINGINLINE void io_uring_prep_msg_ring(struct io_uring_sqe* sqe,
                                                    int                  fd,
                                                    unsigned int         len,
                                                    __u64                data,
                                                    unsigned int         flags) noexcept {
...
IOURINGINLINE void io_uring_prep_msg_ring_fd(struct io_uring_sqe* sqe,
                                             int                  fd,
                                             int                  source_fd,
                                             int                  target_fd,
                                             __u64                data,
    ...
    io_uring_prep_rw(IORING_OP_MSG_RING,
                     sqe,
                     fd,
                     uring_reinterpret_cast(void*, uring_reinterpret_cast(uintptr_t, IORING_MSG_SEND_FD)),
                     0,
                     data);
    sqe->addr3 = source_fd;
    /* offset by 1 for allocation */
    if (uring_static_cast(unsigned int, target_fd) == IORING_FILE_INDEX_ALLOC)
        target_fd--;
    internal__io_uring_set_target_fixed_file(sqe, target_fd);
    sqe->msg_ring_flags = flags;
}
...
IOURINGINLINE void io_uring_prep_getxattr(struct io_uring_sqe* sqe,
    ...
    io_uring_prep_rw(IORING_OP_GETXATTR,
                     sqe,
                     0,
                     name,
                     len,
                     uring_reinterpret_cast(__u64, uring_reinterpret_cast(uintptr_t, value)));
    sqe->addr3       = uring_reinterpret_cast(__u64, uring_reinterpret_cast(uintptr_t, path));
    sqe->xattr_flags = 0;
}
...
IOURINGINLINE void io_uring_prep_fgetxattr(struct io_uring_sqe* sqe,
                                           ...
                                           char*                value,
                                           unsigned int         len) noexcept {
    io_uring_prep_rw(IORING_OP_FGETXATTR,
                     sqe,
                     fd,
                     name,
                     len,
...
IOURINGINLINE void io_uring_prep_fsetxattr(struct io_uring_sqe* sqe,
                                           ...
                                           int                  flags,
                                           unsigned int         len) noexcept {
    io_uring_prep_rw(IORING_OP_FSETXATTR,
                     sqe,
                     fd,
                     name,
                     len,
...
constexpr IOURINGINLINE void io_uring_prep_socket(struct io_uring_sqe* sqe,
                                                  int                  domain,
                                                  int                  type,
                                                  int                  protocol,
                                                  unsigned int         flags) noexcept {
    io_uring_prep_rw(IORING_OP_SOCKET, sqe, domain, nullptr, protocol, type);
    sqe->rw_flags = uring_static_cast(__kernel_rwf_t, flags);
}

constexpr IOURINGINLINE void io_uring_prep_socket_direct(struct io_uring_sqe* sqe,
                                                         int                  domain,
                                                         int                  type,
                                                         int                  protocol,
                                                         unsigned             file_index,
...
constexpr IOURINGINLINE void io_uring_prep_socket_direct_alloc(struct io_uring_sqe* sqe,
                                                               int                  domain,
                                                               int                  type,
                                                               int                  protocol,
                                                               unsigned int         flags) noexcept {
 ...
 */
constexpr IOURINGINLINE void io_uring_prep_cmd_sock(struct io_uring_sqe* sqe,
                                                    int                  cmd_op,
                                                    int                  fd,
                                                    int                  level,
                                                    int                  optname,
 ...
 */
IOURINGINLINE unsigned io_uring_sq_space_left(const struct io_uring* ring) noexcept {
 ...
 * this feature.
 */
IOURINGINLINE int io_uring_sqring_wait(struct io_uring* ring) noexcept {
    if (!(ring->flags & IORING_SETUP_SQPOLL))
        return 0;
    if (io_uring_sq_space_left(ring))
        return 0;

    return internal__io_uring_sqring_wait(ring);
}
 ...
 */
nodiscard constexpr IOURINGINLINE bool io_uring_cq_eventfd_enabled(const struct io_uring* ring) noexcept {
    if (!ring->cq.kflags)
        return true;
 ...
 * the ring.
 */
IOURINGINLINE int io_uring_cq_eventfd_toggle(struct io_uring* ring, bool enabled) noexcept {
    uint32_t flags;
 ...
 */
IOURINGINLINE int
io_uring_wait_cqe_nr(struct io_uring* ring, struct io_uring_cqe** cqe_ptr, unsigned wait_nr) noexcept {
 ...
 */
IOURINGINLINE int io_uring_peek_cqe(struct io_uring* ring, struct io_uring_cqe** cqe_ptr) noexcept {
 ...
 * cqe_ptr filled in on success, -errno on failure.
 */
IOURINGINLINE int io_uring_wait_cqe(struct io_uring* ring, struct io_uring_cqe** cqe_ptr) noexcept {
    if (!internal__io_uring_peek_cqe(ring, cqe_ptr, nullptr) && *cqe_ptr)
        return 0;
 ...
 */
constexpr IOURINGINLINE int io_uring_buf_ring_mask(__u32 ring_entries) noexcept {
    return uring_static_cast(int, ring_entries) - 1;
}
...
IOURINGINLINE void io_uring_buf_ring_add(struct io_uring_buf_ring* br,
    ...
    buf->addr = uring_reinterpret_cast(unsigned long, uring_reinterpret_cast(uintptr_t, addr));
    buf->len  = len;
    buf->bid  = bid;
}
 ...
 * buffers.
 */
IOURINGINLINE void io_uring_buf_ring_advance(struct io_uring_buf_ring* br, int count) noexcept {
    const unsigned short new_tail = br->tail + count;
    io_uring_smp_store_release(&br->tail, new_tail);
}

IOURINGINLINE void internal__io_uring_buf_ring_cq_advance(struct io_uring*          ring,
                                                          struct io_uring_buf_ring* br,
                                                          int                       cq_count,
 ...
 */
IOURINGINLINE void
io_uring_buf_ring_cq_advance(struct io_uring* ring, struct io_uring_buf_ring* br, int count) noexcept {
    internal__io_uring_buf_ring_cq_advance(ring, br, count, count);
}
