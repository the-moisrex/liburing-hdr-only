...
static inline int io_uring_register_buffers_update_tag(struct io_uring *ring, unsigned off,
...
static inline int io_uring_register_buffers_tags(struct io_uring *ring,
                                                 const struct iovec *iovecs,
                                                 const __u64 *tags,
                                                 unsigned nr) noexcept {
...
static inline int io_uring_register_buffers_sparse(struct io_uring *ring, unsigned nr) noexcept {
    struct io_uring_rsrc_register reg = {
...
static inline int io_uring_register_buffers(struct io_uring *ring, const struct iovec *iovecs,
                                            unsigned nr_iovecs) noexcept {
    return do_register(ring, IORING_REGISTER_BUFFERS, iovecs, nr_iovecs);
}

static inline int io_uring_unregister_buffers(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_BUFFERS, NULL, 0);
}

static inline int io_uring_register_files_update_tag(struct io_uring *ring, unsigned off,
 ...
 */
static inline int io_uring_register_files_update(struct io_uring *ring, unsigned off,
...
static inline int io_uring_register_files_sparse(struct io_uring *ring, unsigned nr)  noexcept {
    struct io_uring_rsrc_register reg = {
            .flags = IORING_RSRC_REGISTER_SPARSE,
            .nr = nr,
    };
...
static inline int io_uring_register_files_tags(struct io_uring *ring, const int *files,
...
static inline int io_uring_register_files(struct io_uring *ring, const int *files,
                                          unsigned nr_files) noexcept {
    int ret, did_increase = 0;
...
static inline int io_uring_unregister_files(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_FILES, NULL, 0);
}

static inline int io_uring_register_eventfd(struct io_uring *ring, int event_fd) noexcept {
    return do_register(ring, IORING_REGISTER_EVENTFD, &event_fd, 1);
}

static inline int io_uring_unregister_eventfd(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_EVENTFD, NULL, 0);
}

static inline int io_uring_register_eventfd_async(struct io_uring *ring, int event_fd) noexcept {
    return do_register(ring, IORING_REGISTER_EVENTFD_ASYNC, &event_fd, 1);
}

static inline int io_uring_register_probe(struct io_uring *ring, struct io_uring_probe *p,
                                          unsigned int nr_ops) noexcept {
    return do_register(ring, IORING_REGISTER_PROBE, p, nr_ops);
}

static inline int io_uring_register_personality(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_REGISTER_PERSONALITY, NULL, 0);
}

static inline int io_uring_unregister_personality(struct io_uring *ring, int id) noexcept {
    return do_register(ring, IORING_UNREGISTER_PERSONALITY, NULL, id);
}

static inline int io_uring_register_restrictions(struct io_uring *ring,
                                                 struct io_uring_restriction *res,
                                                 unsigned int nr_res) noexcept {
    return do_register(ring, IORING_REGISTER_RESTRICTIONS, res, nr_res);
}

static inline int io_uring_enable_rings(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_REGISTER_ENABLE_RINGS, NULL, 0);
}

static inline int io_uring_register_iowq_aff(struct io_uring *ring, size_t cpusz,
...
static inline int io_uring_unregister_iowq_aff(struct io_uring *ring) noexcept {
    return do_register(ring, IORING_UNREGISTER_IOWQ_AFF, NULL, 0);
}

static inline int io_uring_register_iowq_max_workers(struct io_uring *ring, unsigned int *val) noexcept {
    return do_register(ring, IORING_REGISTER_IOWQ_MAX_WORKERS, val, 2);
}

static inline int io_uring_register_ring_fd(struct io_uring *ring) noexcept {
...
static inline int io_uring_unregister_ring_fd(struct io_uring *ring) noexcept {
    struct io_uring_rsrc_update up = {
...
static inline int io_uring_close_ring_fd(struct io_uring *ring) noexcept {
    if (!(ring->features & IORING_FEAT_REG_REG_RING))
        return -EOPNOTSUPP;
    if (!(ring->int_flags & INT_FLAG_REG_RING))
        return -EINVAL;
 ...
 */
static inline struct io_uring_probe *io_uring_get_probe_ring(struct io_uring *ring) noexcept {
...
struct io_uring_probe *io_uring_get_probe(void)noexcept;

/*
 * frees a probe allocated through io_uring_get_probe() or
 * io_uring_get_probe_ring()
 */
void io_uring_free_probe(struct io_uring_probe *probe) noexcept;
