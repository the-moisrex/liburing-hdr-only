...
static int __test_io(const char*      file,
...
#ifdef VERBOSE
    fprintf(stdout, "%s: start %d/%d/%d/%d/%d: ", __FUNCTION__, write, buffered, sqthread, fixed, nonvec);
#endif
    if (write)
        open_flags = O_WRONLY;
    else
    ...
    for (i = 0; i < BUFFERS; i++) {
        ...
        if (write) {
            ...
            if (fixed && (i & 1))
                do_fixed = 0;
            if (do_fixed) {
                io_uring_prep_write_fixed(sqe, use_fd, vecs[i].iov_base, vecs[i].iov_len, offset, i);
            } else if (nonvec) {
                io_uring_prep_write(sqe, use_fd, vecs[i].iov_base, vecs[i].iov_len, offset);
            } else {
                io_uring_prep_writev(sqe, use_fd, &vecs[i], 1, offset);
            }
        } else {
            int do_fixed = fixed;
            int use_fd   = fd;
            ...
            if (fixed && (i & 1))
                do_fixed = 0;
            if (do_fixed) {
                io_uring_prep_read_fixed(sqe, use_fd, vecs[i].iov_base, vecs[i].iov_len, offset, i);
            } else if (nonvec) {
                io_uring_prep_read(sqe, use_fd, vecs[i].iov_base, vecs[i].iov_len, offset);
            } else {
                io_uring_prep_readv(sqe, use_fd, &vecs[i], 1, offset);
            }
        }
        sqe->user_data = i;
        if (sqthread)
...
static int
test_io(const char* file, int write, int buffered, int sqthread, int fixed, int nonvec, int exp_len) {
    struct io_uring ring;
    ...
    ret = __test_io(file, &ring, write, buffered, sqthread, fixed, nonvec, 0, 0, exp_len);
...
static int test_buf_select_short(const char* filename, int nonvec) {
    ...
    for (i = 0; i < BUFFERS; i++) {
        sqe = io_uring_get_sqe(&ring);
        io_uring_prep_provide_buffers(sqe, vecs[i].iov_base, vecs[i].iov_len / 2, 1, 1, i);
        if (!exp_len)
...
static int provide_buffers_iovec(struct io_uring* ring, int bgid) {
    ...
    for (i = 0; i < BUFFERS; i++) {
        sqe = io_uring_get_sqe(ring);
        io_uring_prep_provide_buffers(sqe, vecs[i].iov_base, vecs[i].iov_len, 1, bgid, i);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        snprintf(buf, sizeof(buf), ".basic-rw-%u-%u", (unsigned) rand(), (unsigned) getpid());
        fname = buf;
    ...
    for (i = 0; i < nr; i++) {
        ...
        ret = test_io(fname, write, buffered, sqthread, fixed, nonvec, BS);
        if (ret) {
    ...
    for (i = 0; i < nr; i++) {
        ...
        ret = test_io(fname, write, buffered, sqthread, fixed, nonvec, -1);
        if (ret) {
            fprintf(stderr, "test_io failed %d/%d/%d/%d/%d\n", write, buffered, sqthread, fixed, nonvec);
            goto err;
        }
