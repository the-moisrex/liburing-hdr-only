...
static int expect_fail(int fd, unsigned int opcode, void* arg, unsigned int nr_args, int error, int error2) {
    int ret;
    ...
    if (ret >= 0) {
        ...
        if (opcode == IORING_REGISTER_BUFFERS) {
            ret2 = io_uring_register(fd, IORING_UNREGISTER_BUFFERS, 0, 0);
        } else if (opcode == IORING_REGISTER_FILES) {
            ret2 = io_uring_register(fd, IORING_UNREGISTER_FILES, 0, 0);
        }
        if (ret2) {
...
static int test_max_fds(int uring_fd) {
     ...
     */
    fd_as = mmap(NULL, UINT_MAX * sizeof(int), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (fd_as == MAP_FAILED) {
        if (errno == ENOMEM)
            return 0;
        perror("mmap fd_as");
    ...
    fds = mmap(fd_as, 128 * 1024 * 1024, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fdtable_fd, 0);
    if (fds == MAP_FAILED) {
        perror("mmap fdtable");
        exit(1);
    }
    ...
    for (i = 0; i < nr_maps; i++) {
        ...
        fds = mmap(fds, 128 * 1024 * 1024, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fdtable_fd, 0);
        if (fds == MAP_FAILED) {
    ...
    nr_fds = UINT_MAX;
    while (nr_fds) {
        ret = io_uring_register(uring_fd, IORING_REGISTER_FILES, fd_as, nr_fds);
        if (ret != 0) {
            nr_fds /= 2;
            continue;
        }
...
static int test_iovec_size(int fd) {
    ...
    buf = mmap(NULL, 2 * pagesize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    if (buf == MAP_FAILED) {
        ...
        if (ret < 0) {
            ...
            ret = io_uring_register(fd, IORING_UNREGISTER_BUFFERS, 0, 0);
            if (ret < 0) {
