...
static int test_underprep_fail(bool hardlink, bool drain, bool link_last, int link_size, int fail_idx) {
    ...
    for (link_size = 0; link_size < 3; link_size++) {
        for (fail_idx = 0; fail_idx < link_size; fail_idx++) {
            for (i = 0; i < 8; i++) {
                ...
                ret = test_underprep_fail(hardlink, drain, link_last, link_size, fail_idx);
                if (!ret)
                    continue;

                fprintf(stderr,
                        "failed %d, hard %d, drain %d,"
