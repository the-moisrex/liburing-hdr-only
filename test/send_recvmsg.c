...
static int
recv_prep(struct io_uring* ring, int* sockfd, struct iovec iov[], int iov_count, int bgid, int async) {
...
static int do_recvmsg(struct io_uring* ring, char buf[MAX_MSG + 1], struct recv_data* rd) {
...
static void init_iov(struct iovec iov[MAX_IOV_COUNT], int iov_to_use, char buf[MAX_MSG + 1]) {
...
static void* recv_fn(void* data) {
    ...
    if ((rd->buf_ring || rd->buf_select) && !rd->no_buf_add) {
        if (rd->buf_ring) {
            ...
            io_uring_buf_ring_add(br, buf, sizeof(buf), BUF_BID, io_uring_buf_ring_mask(1), 0);
            ...
            sqe = io_uring_get_sqe(&ring);
            io_uring_prep_provide_buffers(sqe, buf, sizeof(buf) - 1, 1, BUF_BGID, BUF_BID);
            sqe->user_data = ++ud;
            ret            = io_uring_submit(&ring);
            if (ret != 1) {
...
static int test(int buf_select, int buf_ring, int no_buf_add, int iov_count, int async) {
    struct recv_data    rd;
