...
static int provide_buffers(struct io_uring* ring) {
    ...
    for (i = 0; i < BUFFERS; i++) {
        ...
        io_uring_prep_provide_buffers(sqe, vecs[i].iov_base, vecs[i].iov_len, 1, 1, i);
    }
...
static int
__test_io(const char* file, struct io_uring* ring, int write, int sqthread, int fixed, int buf_select) {
    struct io_uring_sqe* sqe;
    struct io_uring_cqe* cqe;
    int                  open_flags;
    int                  i, fd = -1, ret;
...
static int test_io(const char* file, int write, int sqthread, int fixed, int buf_select, int defer) {
    struct io_uring ring;
    ...
    if (defer)
        ring_flags |= IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN;
