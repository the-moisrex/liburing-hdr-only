...
static int test_link_fail(struct io_uring* ring, int nr, int fail_idx) {
    ...
    if (!cqe->res || cqe->user_data != fail_idx) {
        fprintf(stderr,
                "got: user_data %d res %d, expected data: %d\n",
                (int) cqe->user_data,
                cqe->res,
                fail_idx);
        goto err;
...
static int test_ltimeout_cancel(struct io_uring* ring, int nr, int tout_idx, bool async, int fail_idx) {
...
static int test_ltimeout_fire(struct io_uring* ring, bool async, bool skip_main, bool skip_tout) {
    char                     buf[1];
...
static int test_hardlink(struct io_uring* ring, int nr, int fail_idx, int skip_idx, bool hardlink_last) {
    struct io_uring_cqe* cqe;
    struct io_uring_sqe* sqe;
    int                  ret, i;
