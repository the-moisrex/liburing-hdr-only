...
static int test_clock_setup(void)
{
	...
	ret = internal__sys_io_uring_register(ring.ring_fd, IORING_REGISTER_CLOCK, NULL, 0);
	if (!ret) {
	...
	cr.clockid = -1;
	ret = internal__sys_io_uring_register(ring.ring_fd, IORING_REGISTER_CLOCK, &cr, 0);
	if (ret != -EINVAL) {
