...
static int do_splice_op(struct io_uring* ring,
    ...
    do {
        ...
        io_uring_prep_splice(sqe, fd_in, off_in, fd_out, off_out, len, splice_flags);
        sqe->flags |= sqe_flags;
        sqe->user_data = 42;
        sqe->opcode    = opcode;
...
static int
do_splice(struct io_uring* ring, int fd_in, loff_t off_in, int fd_out, loff_t off_out, unsigned int len) {
    return do_splice_op(ring, fd_in, off_in, fd_out, off_out, len, IORING_OP_SPLICE);
}

static int do_tee(struct io_uring* ring, int fd_in, int fd_out, unsigned int len) {
...
static int splice_to_pipe(struct io_uring* ring, struct test_ctx* ctx) {
    ...
    ret = check_content(ctx->real_pipe1[0], ctx->buf_out, BUF_SIZE, ctx->buf_in);
    if (ret)
        return ret;
    ...
    return check_content(ctx->real_pipe1[0], ctx->buf_out, BUF_SIZE, ctx->buf_in);
}
...
static int splice_from_pipe(struct io_uring* ring, struct test_ctx* ctx) {
    ...
    if (ret)
        return ret;
    ret = check_content(ctx->real_fd_out, ctx->buf_out, BUF_SIZE, ctx->buf_in);
    if (ret)
        return ret;
...
static int splice_pipe_to_pipe(struct io_uring* ring, struct test_ctx* ctx) {
    ...
    return check_content(ctx->real_pipe2[0], ctx->buf_out, BUF_SIZE, ctx->buf_in);
}
...
static int fail_tee_offset(struct io_uring* ring, struct test_ctx* ctx) {
    ...
    ret = do_splice_op(ring, ctx->pipe2[0], 0, ctx->pipe1[1], -1, BUF_SIZE, IORING_OP_TEE);
    if (ret != -ESPIPE && ret != -EINVAL)
        return ret;
...
static int check_tee(struct io_uring* ring, struct test_ctx* ctx) {
    ...
    ret = check_content(ctx->real_pipe2[0], ctx->buf_out, BUF_SIZE, ctx->buf_in);
    if (ret) {
