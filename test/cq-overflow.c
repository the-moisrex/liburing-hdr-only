...
static int reap_events(struct io_uring* ring, unsigned nr_events, int do_wait) {
    ...
    for (i = 0; i < nr_events; i++) {
        ...
        if (do_wait && ret == -EBADR) {
            unsigned int this_drop = *ring->cq.koverflow - start_overflow;
 ...
 */
static int test_overflow_handling(bool batch, int cqe_multiple, bool poll, bool defer) {
    struct io_uring        ring;
    struct io_uring_params p;
    ...
    for (i = 0; i < LOOPS; i++) {
        ...
        if (io_uring_cq_has_overflow(&ring)) {
            ...
            if (ret != 0) {
                fprintf(stderr, "io_uring_get_events returned %d\n", ret);
                goto err;
            }
        } else if (!cqe_dropped) {
            for (j = 0; j < queued; j++) {
        ...
        if (batch) {
            ...
            if (ret < 0) {
                fprintf(stderr, "io_uring_peek_batch_cqe failed %d\n", ret);
                goto err;
            }
            cqe_count = ret;
        }
