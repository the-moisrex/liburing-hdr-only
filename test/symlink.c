...
static int test_link_contents(const char* linkname, const char* expected_contents) {
    char buf[128];
    ...
    if (strncmp(buf, expected_contents, 128)) {
        fprintf(stderr, "link contents differs from expected: '%s' vs '%s'", buf, expected_contents);
        return -1;
    }
    return 0;
}
