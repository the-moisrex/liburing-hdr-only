 ...
 */
static int register_rsrc(struct io_uring* ring, int type, int nr, const void* arg, const __u64* tags) {
 ...
 * but they may change, so hand-code to specifically test this ABI.
 */
static int update_rsrc(struct io_uring* ring, int type, int nr, int off, const void* arg, const __u64* tags) {
...
static int test_buffers_update(void) {
    ...
    ret = update_rsrc(&ring, TEST_IORING_RSRC_BUFFER, 1, buf_idx, &vecs[buf_idx], &tags[buf_idx]);
    if (ret != 1) {
...
static int test_files(int ring_flags) {
    ...
    if (cqe->user_data != tags[off]) {
        fprintf(stderr, "data %lx != %lx\n", (unsigned long) cqe->user_data, (unsigned long) tags[off]);
        return 1;
    }
    io_uring_cqe_seen(&ring, cqe);
