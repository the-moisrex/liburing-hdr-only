...
static int
test_update_multiring(struct io_uring* r1, struct io_uring* r2, struct io_uring* r3, int do_unreg) {
    int *fds, *newfds;

    fds    = open_files(10, 0, 0);
    newfds = open_files(10, 0, 1);

    if (io_uring_register_files(r1, fds, 10) || io_uring_register_files(r2, fds, 10) ||
    ...
    if (io_uring_unregister_files(r1) || io_uring_unregister_files(r2) || io_uring_unregister_files(r3)) {
...
static int test_update_no_table(void) {
    ...
    ret = cqe->res;
    io_uring_cqe_seen(&ring, cqe);
    if (ret != -EMFILE && ret != -EINVAL && ret != -EOVERFLOW && ret != -ENXIO) {
...
int main(int argc, char* argv[]) {
    ...
    if (io_uring_queue_init(8, &r1, 0) || io_uring_queue_init(8, &r2, 0) || io_uring_queue_init(8, &r3, 0)) {
