...
static int recv_prep(struct io_uring* ring, struct iovec* iov, int* sock, struct recv_data* rd) {
    ...
    if (!rd->use_recvmsg) {
        io_uring_prep_recv(sqe, sockfd, iov->iov_base, iov->iov_len, MSG_WAITALL);
    } else {
